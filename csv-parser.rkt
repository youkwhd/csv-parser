    ...
    [else
      ...
      (define parsed-fields 
        (let parse-line ([str : String str]
          ...
          (cond
            [(cdr field)
             (cons (append fields (list (trim-quote-capsulated (transform-double-quote (remove-quote-capsulated (caar field)))))) (cdar field))]
            [else
              (parse-line (cdar field) (append fields (list (trim-quote-capsulated (transform-double-quote (remove-quote-capsulated (caar field)))))))])))
